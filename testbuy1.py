...
obj1=th.Tradehull("apikey","secret key",'yes')
kite=obj1.kite
y=0
x=0
z=2
...
for name in watchlist:
    ...
    for i in range (15,total_len):
        ...
        if test_data['ema200']<test_data['low'] and test_data['ema150']<test_data['low'] and upside==0 :
            upside=10
            dwside=0
        if str(test_data['date'])[11:19]=='09:15:00':
        ...
        if y==10 and test_data['low']<(sell_price-sl) and str(test_data['date'])[11:19]!=time_entr and m==0:
           exit_p=sell_price-sl
           exit_t=test_data['date']
           pnlper=-((sell_price-test_data['close'])/test_data['close'])*100
           m=10
...
k.to_csv('all  buy market stock backtest.csv')