...
watchlist=['BANDHANBNK','RBLBANK','ADANIPOWER','BHEL','GODREJCP','VEDL','SAIL','BANKBARODA','TATAMOTORS','INDUSINDBK']
watchlist = ['ADANIPORTS', 'APOLLOTYRE', 'ASHOKLEY', 'AXISBANK', 'BAJFINANCE', 'BAJAJFINSV', 'BANDHANBNK', 'BANKBARODA', 'BHEL', 'BPCL', 'CANFINHOME', 'CANBK', 'CHOLAFIN', 'COFORGE', 'DLF', 'ESCORTS', 'FEDERALBNK', 'GODREJPROP', 'GRASIM', 'HINDALCO', 'HDFC', 'ICICIBANK', 'ICICIPRULI', 'IDFCFIRSTB', 'IBULHSGFIN', 'INDUSINDBK', 'JSWSTEEL', 'JINDALSTEL', 'L&TFH', 'LICHSGFIN', 'M&MFIN', 'MANAPPURAM', 'MARUTI', 'MFSL', 'MOTHERSUMI', 'MUTHOOTFIN', 'NMDC', 'NAM-INDIA', 'PEL', 'PFC', 'RBLBANK', 'RADICO', 'SRTRANSFIN', 'SBIN', 'SAIL', 'TATAMOTORS', 'TATASTEEL', 'UJJIVAN', 'VEDL', 'IDEA']
...
watchlist=['CIPLA', 'COALINDIA', 'DIVISLAB', 'DRREDDY', 'EICHERMOT', 'GAIL', 'GRASIM', 'HCLTECH', 'HDFCBANK', 'HDFCLIFE', 'HEROMOTOCO', 'HINDALCO']
watchlist=['NIFTY 50']   
'''fut_watchlist=[]
for item in watchlist:
...
all_trade1=[]
order_dict1={'script':None,'buy/sell':None,'date':None,'entry':None,'exit':None,'stoploss':None ,'exit_time':None,'lot_size':25,'percent_return':None,'gap':None,'net_change':None,'can_color':None, 'high1':None,'low1':None,'pre_day_move':None,'preday22':None,'day':None,'upside1':None,'down_side':None,'ema_trade_price':None}
high1=0
low1=1000000
m=0
multi=0.15
...
for name in watchlist:
    ...
    for i in range (15,total_len):
        ...
        if str(test_data['date'])[11:19]==time_entr:
            ...
            for p in range (1,total_len2):
                ...
                if trade_date==str(test_data5['date'])[0:10]:
                    ...
                    order_dict1['net_change']=(close_day-close2)/close2*100
                    break
        if str(test_data['date'])[11:19]==time_entr and y==0:
            ...
            sell_price=test_data['close']
            order_dict1['down_side']=dwside
            order_dict1['upside1']=upside
            order_dict1['ema_trade_price']=abs(test_data['ema200']-sell_price)/sell_price*100
        ...
        if y==10 and test_data['high']>(sell_price+sl) and str(test_data['date'])[11:19]!=time_entr and m==0:
...
k.to_csv('all  sell market stock xxxx21 backtest.csv')